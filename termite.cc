...
gboolean key_press_cb(VteTerminal *vte, GdkEventKey *event, keybind_info *info) {
    ...
    if (info->select.mode != vi_mode::insert) {
        ...
        int old_movement_count = movement_count;
        switch (event->keyval) {
            case GDK_KEY_Escape:
            case GDK_KEY_q:
                exit_command_mode(vte, &info->select);
            ...
            case GDK_KEY_8:
            case GDK_KEY_9:
                movement_count = movement_count * 10 + event->keyval - GDK_KEY_0;
                break;
            case GDK_KEY_Left:
            case GDK_KEY_h:
                move(vte, &info->select, -(movement_count?movement_count:1), 0);
                movement_count = 0;
                break;
            case GDK_KEY_Down:
            case GDK_KEY_j:
                move(vte, &info->select, 0, (movement_count?movement_count:1));
                movement_count = 0;
                break;
            case GDK_KEY_Up:
            case GDK_KEY_k:
        ...
        if (movement_count && old_movement_count == movement_count) {
            movement_count = 0;
        }
        return TRUE;
    }
